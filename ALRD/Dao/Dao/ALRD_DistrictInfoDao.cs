...
namespace SH.Dao
{
    public partial class ALRD_DistrictInfoDao : BaseDao, IALRD_DistrictInfoDao
    {

        #region Constructors

        private string ClassName = "ALRD_DistrictInfoDao";

        public ALRD_DistrictInfoDao()
            : base()
        {
        ...
        public static void FillParameters(ALRD_DistrictInfoEntity entity, DbCommand cmd, Database Database, bool forDelete = false)
        {
        ...
        long IALRD_DistrictInfoDao.Delete(ALRD_DistrictInfoEntity entity)
        {
            long returnCode = -99;
            const string SP = "alrd_districtinfo_Delete";
        ...
        long IALRD_DistrictInfoDao.Update(ALRD_DistrictInfoEntity entity, bool isPertial = false)
        {
            long returnCode = -99;

            string SP;
            if (!isPertial)
                SP = "alrd_districtinfo_Update";
            else
                SP = "alrd_districtinfo_PertialUpdate";
        ...
        long IALRD_DistrictInfoDao.Add(ALRD_DistrictInfoEntity entity)
        {
            long returnCode = -99;
            const string SP = "alrd_districtinfo_Insert";
        ...
        long IALRD_DistrictInfoDao.SaveList(IList<ALRD_DistrictInfoEntity> list)
        {
            ...
            try
            {
                ...
                foreach (ALRD_DistrictInfoEntity item in list){if (item.CurrentState == SH.Entity.BaseEntity.EntityState.Added) { listAdded.Add(item); }}
                foreach (ALRD_DistrictInfoEntity item in list) { if (item.CurrentState == SH.Entity.BaseEntity.EntityState.Changed) { listUpdated.Add(item); } }
                foreach (ALRD_DistrictInfoEntity item in list) { if (item.CurrentState == SH.Entity.BaseEntity.EntityState.Deleted) { listDeleted.Add(item); } }
        ...
        IList<ALRD_DistrictInfoEntity> IALRD_DistrictInfoDao.GetAll(ALRD_DistrictInfoEntity entity)
        {
            try
            {
                const string SP = "alrd_districtinfo_GetAll";
                using (DbCommand cmd = Database.GetStoredProcCommand(SP))
                {
                    ...
                    using (IDataReader reader = Database.ExecuteReader(cmd))
                    {
                        while (reader.Read())
                        {
                            itemList.Add(new ALRD_DistrictInfoEntity(reader));
                        }
                    }
                    return itemList;
                }
